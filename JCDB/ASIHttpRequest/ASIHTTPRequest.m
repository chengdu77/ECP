...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		ASIRequestTimedOutError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"本次网络请求超时，请稍后再试",NSLocalizedDescriptionKey,nil]];  
		...
		ASIRequestCancelledError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:ASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"本次网络请求已被取消",NSLocalizedDescriptionKey,nil]];
